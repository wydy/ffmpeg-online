...
const defaultArgs = JSON.stringify(["-i", "input.mp4", "-c:v", "libx265", "-crf", "28", "output.mp4"], null, 2);
...
export default function Workspace({ ffmpeg: _ffmpeg }: WorkspaceProps) {
  ...
  const refreshDir = async (curPath: string) => {
    if (ffmpeg.loaded) {
      setNodes(
        (await ffmpeg.listDir(curPath)).filter(({ name }) => name !== [".","tmp","home","proc","dev"])
      );
    }
  };
  ...
  const onFileUpload =
    ...
    async ({ target: { files } }: ChangeEvent<HTMLInputElement>) => {
      for (let i = 0; i < files.length; i++) {
        ...
        await ffmpeg.writeFile(`${path}/input.mp4`, data);
      }
      refreshDir(path);
    };
